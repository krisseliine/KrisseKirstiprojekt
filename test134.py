...
##            algus(j,0)
    ...
    if n >= len(küsitav):
        ...
        lõpp = ttk.Button(raam6, text="Uuesti!", command=lambda: algus(-1,0))
...
#            break
    ...
    #raam2.focus_set()
    nupp = ttk.Button(raam2, text="TEST", command=lambda: tõlk(n,küsitav,vastus, raam2,j)).pack()
    if len(küsitav) == 0:
        global raam7
        raam7=Tk()
        raam7.title("Viga!")
       # messagebox.showinfo(title="Viga",message="Oled jätnud sõnade faili tühjaks, lisa sõnad!")
        veateade = ttk.Label(raam7, text="Oled jätnud sõnade faili tühjaks, lisa sõnad!")
        veateade.grid(column=0, row=0, padx=5, pady=5, sticky=(E,N,W))
 
        lõpp = ttk.Button(raam7, text="Uuesti!", command=lambda: algus(-1,0))
        lõpp.grid(column=1, row=1, padx=5, pady=5, sticky=(N, S, W, E))
 
        panenkinni = ttk.Button(raam7, text="Sule aken", command= läbi2)
...
def algus(j,n):
 ...
 #   tahvel = Canvas(raam, image=pilt)
    logo = BitmapImage(pilt, foreground='red')
    e = Label(image=logo).grid()
 
    #tahvel.place(anchor=CENTER)
    #tahvel.grid()
    kuidasSisestada = ttk.Label(e, text="Sisesta sõnad faili vastavalt: cat-kass dog-koer")
    ...
    #background.pack()
    failinimi2 = ttk.Label(e, text="Sisesta faili nimi, mis on samas kaustas käesoleva programmiga (NT:('sonad.txt')):")
    failinimi2.grid(column=0, row=1, padx=5, pady=5, sticky=(N, E, W))
 
    esteng = ttk.Label(e, text="Kui soovid, et programm küsiks inglise keeles, sisesta: ING-EST ja vastupidi: EST-ING:")
    esteng.grid(column=0, row=2, padx=5, pady=5, sticky=(N, E, W))
 
#lahtrid
    küsibfailinime = ttk.Entry(e)
...
#Teen nupu ning annan commandi talle selle, mis ta teeb kui vajutatakse nuppu, ehk siis funktsiooni
    ...
    nupp = ttk.Button(e, text="Hakkame pihta!", command=lambda:proov(j,küsibfailinime,esteng1,n))
